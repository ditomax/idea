...
2. Determine where things stand:
   ...
   - cards exist, none evaluated → collect can continue, or **evaluate** can start — ask which;
   - evaluations exist, no shortlist → evaluate continues (more cards) or closes with the shortlist — ask which;
   - shortlist `done` → say so, offer: another collect round, a new committee round (`10-shortlist.v2.md`), or hand over to maquette ("open the maquette suite and type start — it will find the shortlist").
...
Then create `00-idea.md` from `templates/00-idea.md` with `revision: 1`, budgets (collect 45 min per session, evaluate 30 min per card, shortlist 30 min), `profile` set, the log line `start`, and run collect.

## Running a stage

1. Tell the user which "team member" comes now, in plain words: interviewer (collect) · committee moderator (evaluate). One sentence on what they get and how many minutes it takes.
...
5. Ask the user: "Good as it is — next, or redo?" On next: set `status: done` where the stage proposed it (the only field you edit in another skill's file), update the card index and the session log, log the decision. With `git: yes`, commit per RULES §6 and say so in half a sentence.
6. If minutes used exceed the budget by more than 25 %, note it under "Open items for the Director" — data for the retro, not a reprimand.
